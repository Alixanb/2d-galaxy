...
class Canvas {
  ...
  constructor(ref: string, grid: number = 1) {
    ...
    if (!element) throw new Error("Unable to fetch canvas element with string " + ref)
  ...
  place(vec: Vec2) {
      const normalizedX = (vec.x + 1) / 2 * (this.dimensions.x / window.devicePixelRatio || 1);
      const normalizedY = (vec.y + 1) / 2 * (this.dimensions.y / window.devicePixelRatio || 1);
...
class Star {
  ...
  update(blackholes: BlackHole[]) {
    ...
    for (let blackhole of blackholes) {
      ...
      if (distance > blackhole.size / 600) {
        const forceMagnitude = Galaxy.G * ((blackhole.mass) / (distance ** 2)) * Star.kGravity;
  ...
  static getColor(vel: Vec2): string {
    ...
    const value = clamp(
      Math.round((vel.length() / this.MAX_VELOCITY) * 255 * COLOR_MULTIPLICATOR), 
      0, 
      255
    );
...
class BlackHole extends Star {
  ...
  draw(canvas: Canvas) {
    ...
    const outerGlow = ctx.createRadialGradient(
      screenPos.x, screenPos.y, eventHorizonRadius,
...
class Galaxy {
  ...
  constructor(canvas: Canvas, blackholes: BlackHole[], ship?: Ship, nStar: number = 100)  {
    this.canvas = canvas;
    this.stars = [];
  ...
  draw() {
    this.canvas.context.clearRect(0, 0, this.canvas.dimensions.x, this.canvas.dimensions.y)
...
class Ship {
  ...
  constructor(pos: Vec2, spriteThrusterOff: HTMLImageElement, spriteThrusterOn: HTMLImageElement, size: number) {
    this.pos = pos;
  ...
  draw(canvas: Canvas, bhPos: Vec2) {
    ...
    canvas.context.translate(screenPos.x + this.size / 2, screenPos.y + (this.size * this.spriteWidthRatio) / 2);
  ...
  update(blackholes: BlackHole[]) {
    ...
    for (let blackhole of blackholes) {
      ...
      const forceMagnitude = Galaxy.G * ((blackhole.mass) / (distance ** 2)) * Star.kGravity * Ship.MASS;
...
const galaxy = new Galaxy(canvas, [gargantua], discovery, 5000)

Promise.all([loadImage(spriteThrusterOffUrl), loadImage(spriteThrusterOnUrl)])
  .then(([spriteThrusterOff, spriteThrusterOn]) => {
    console.log('Both images are ready!', spriteThrusterOffUrl, spriteThrusterOnUrl);

    galaxy.ship = new Ship(new Vec2(0.8, 0.8), spriteThrusterOff, spriteThrusterOn, 50)
